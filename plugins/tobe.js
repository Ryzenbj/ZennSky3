...
gay = `sayang`
  ...
  let buffer = await (await fetch('http://zekais-api.herokuapp.com/tobecontinue?url=' + img + '&apa=nani')).buffer() // Masukan ApiKeymu disini..
  let res = await fetch('https://raw.githubusercontent.com/herokuapp-com/kuhong-api/main/info/credit.json')
  let json = await res.json()

 await conn.sendFile(m.chat, buffer, 'gambar.jpg', 'To Be Continued', m)
...
handler.help = ['tobe (reply)']
handler.tags = ['maker']
handler.command = /^tobe$/i
handler.owner = false
handler.mods = false
